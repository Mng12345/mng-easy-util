...
export type FitFunc = (...params: number[]) => (...x: number[]) => number
...
const defaultFitFunc: FitFunc = (...params: number[]) => {
  if (params.length <= 1) throw new Error(`the length of params must > 1`)
...
export const fit = ({
  ...
  y,
  initValues,
  fitFunc = defaultFitFunc,
  maxIterations = 500,
  tolerance = 0.01,
}: Options): FitParameter => {
