...
export class GA<
  TIndividual extends AbstractIndividual<TData, TFitness>,
  TData,
  TFitness extends Comparable
> {
  popSize: number = 200
  ...
  cross(): void {
    for (let i = 0; i < this.popSize; i++) {
      ...
      if (p < this.pCross) {
        ...
        const [i1, i2] = this.individualCross(
          this.individuals[index1],
          this.individuals[index2]
        )
   ...
   */
  saveBest(): void {
    const currBestIndividual = min(
      this.individuals,
      (item) => item.fitness!
    )[0]!
    if (
      currBestIndividual.fitness!.compare(this.bestIndividual!.fitness!) < 0
    ) {
