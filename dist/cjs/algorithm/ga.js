...
class GA {
    ...
    initPops() {
        ...
        this.bestIndividual = min_max_1.min(this.individuals, (item) => item.fitness)[0];
    }
    cross() {
        for (let i = 0; i < this.popSize; i++) {
            const p = Math.random();
            if (p < this.pCross) {
                const [index1, index2, _] = math_1.randomNIntNotRepeat(0, this.popSize, 2);
    ...
    mutate() {
        for (let i = 0; i < this.popSize; i++) {
            const p = Math.random();
            if (p < this.pMutate) {
                const index = math_1.randomInt(0, this.popSize);
    ...
    select() {
        ...
        const leftItems = math_1.sample(this.individuals.slice(0, first20), last80);
        const rightItems = math_1.sample(this.individuals.slice(first20), first20);
     ...
     */
    saveBest() {
        const currBestIndividual = min_max_1.min(this.individuals, (item) => item.fitness)[0];
