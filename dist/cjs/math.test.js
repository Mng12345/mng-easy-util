...
test('convertColorToHexNumberArray', () => {
    const array1 = index_1.math.convertColorToHexNumberArray(0x00ff12);
    expect(array1[0]).toBe(0);
    expect(array1[2]).toBe(15);
    expect(array1[5]).toBe(2);
});
test('convertColorToHexCharArray', () => {
    const array1 = index_1.math.convertColorToHexCharArray(0x00ff12);
    expect(array1[0]).toBe('0');
    expect(array1[2]).toBe('f');
    expect(array1[5]).toBe('2');
});
test('parseHexColorToNumber', () => {
    const value = index_1.math.parseHexColorToNumber('0x00ff12');
    expect(value).toBe(0x00ff12);
});
test('ma', () => {
    const data = [1, 2, 3, 4, 5];
    const n = 3;
    const ma = index_1.math.ma(data, 3);
