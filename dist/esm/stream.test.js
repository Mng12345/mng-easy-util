...
test('filter & index', () => {
    ...
    stream = Stream.of([1, 2, 3]);
    const filteredSumByIndex = sum(stream.filter((item, index) => index <= 1).collect());
...
test('stream & flatMap & groupBy', () => {
    const dict = Stream.of([1, 2, 3, 4, 5, 6, 6, 7, 7, 8, 8, 8]).groupBy(item => item).toDict();
    console.log(`dict:`, JSON.stringify(dict));
    Stream.of([1, 2, 3, 4, 5, 6, 6, 7, 7, 8, 8, 8]).groupBy(item => item)
        .forEach(item => console.log(item));
    Stream.of([1, 2, 3, 4, 5, 6, 6, 7, 7, 8, 8, 8]).groupBy(item => item)
        .flatMap(item => item[1])
        .forEach(item => console.log(item));
    const flatValue = Stream.of([1, 2, 3, 4, 5, 6, 6, 7, 7, 8, 8, 8]).groupBy(item => item)
        .map(item => item[1]).flatMap(item => item).collect();
    console.log(`flatValue:`, JSON.stringify(flatValue));
    const collapsedValue = Stream.of([1, 2, 3, 4, 5, 6, 6, 7, 7, 8, 8, 8]).groupBy(item => item)
